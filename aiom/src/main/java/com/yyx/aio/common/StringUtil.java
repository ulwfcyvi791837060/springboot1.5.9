...
public class StringUtil {
	...
	public static String getUserTokenId(HttpServletRequest request) {
        ...
        if(cookies != null){
           for (int i = 0; i < cookies.length; i++) {
               Cookie cookie = cookies[i];
               //System.out.println(cookie.getName()+"=="+cookie.getValue());
               if (StringUtil.isNotEmpty(cookie.getName()) && "baoBenTokenId".equals(cookie.getName().toLowerCase())) {
            	   cookieValue = cookie.getValue();
               } else if ("baoBenTokenId".equals(cookie.getName())) {
            	   cookieValue = cookie.getValue();
               }
           }
       }
	...
	/*
	 * @fun 对指定字符串，长度不够指定长度，则在后面补0
	 * @param str
	 * @param length
	 * @param padStr
	 * @return
	 */
	public static String lpadAfter(String str, String length, String padStr) {
		String tmpStr = "";
		int i = 0;
		if (str.length() >= Integer.parseInt(length)) {
			return str;
